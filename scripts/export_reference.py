...
from datasets.motion_chunk_dataset import decode_future_model_space, load_checkpoint_stats_or_file, load_stats
...
def main() -> None:
    ...
    parser.add_argument("--checkpoint", type=str, default=None, help="Optional checkpoint with embedded normalization stats")
    ...
    if not args.already_denormalized:
        if args.checkpoint is not None:
            ...
            fallback = args.stats or checkpoint.get("config", {}).get("data", {}).get("stats_path")
            if fallback is None:
                raise ValueError("--stats is required for old checkpoints without embedded normalization stats")
            mean, std = load_checkpoint_stats_or_file(checkpoint, fallback)
        else:
            if args.stats is None:
                raise ValueError("--stats or --checkpoint is required unless --already_denormalized is set")
